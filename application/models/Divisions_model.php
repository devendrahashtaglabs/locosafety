...
Class Divisions_model extends CI_Model{
	...
	public function insertDivision($data){
		...
		if($insertedId){
			$this->session->set_flashdata('divisionSuccess', 'Office/Division added successfully.');
			redirect('divisions');
		}else{
	...
	public function updateDivision($data){
		...
		if($query){
			$this->session->set_flashdata('updateDivision', 'Office/Division updated successfully.');
	...
	public function deleteDivision($data){
		...
		$query = $this->db->update(DIVISIONTBL, $data); 
		if($query){
			$this->session->set_flashdata('deleteDivision', 'Office/Division Inactivated successfully');
			redirect('divisions');
		}else{
	...
	public function activateDivision($data){
		...
		$query = $this->db->update(DIVISIONTBL, $data); 
		if($query){
			$this->session->set_flashdata('activateDivision', 'Office/Division Activated successfully');
			redirect('divisions');
		}else{
			$this->session->set_flashdata('errorDivision', 'Somthing went worng. Error!!');
			redirect('divisions');
	...
	public function getActiveDivision(){
		$this->db->select('*');
		$this->db->from(DIVISIONTBL);
		$this->db->where('division_status','10');
		$query 	= $this->db->get();
		$result = $query->result(); 
		return $result; 
	}
