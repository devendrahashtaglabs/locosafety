...
//  Preeti -13-11-2019 ------------------------
    public function getSectionList(){
        $ObjDB = new class_db();
        $S = "SELECT master_shop_tbl.shop_name, master_section_tbl.* FROM master_section_tbl LEFT JOIN master_shop_tbl ON master_shop_tbl.shop_id = master_section_tbl.shop_id WHERE master_section_tbl.shop_id IN (SELECT shop_id FROM `user_mapping_tbl` WHERE user_info_id = '".$this->user_info_id."') AND section_status = '10' ORDER BY section_name ASC";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
