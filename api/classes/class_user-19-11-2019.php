...
//    Preeti -14-11-2019----------
    ...
    public function updateHardwareBasicTbl(){
        $ObjDB = new class_db();
        $S = "UPDATE hardware_basic_tbl SET hardware_status = '".$this->ticket_type."' WHERE hardware_id = '".$this->hardware_id."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->ExecuteQuery();
        return $R;
    }
    ...
    public function raisedTicketList(){
        $ObjDB = new class_db();
        $S = "SELECT hardware_basic_tbl.hardware_name,user_details_tbl.user_f_name,user_details_tbl.user_l_name,"
                . "master_section_tbl.section_name,master_shop_tbl.shop_name,"
                . "master_maintenance_section_tbl.maintenance_section_name,"
                . "master_maintenance_shop_tbl.maintenance_shop_name,tickets_tbl.* "
                . "FROM `tickets_tbl` "
    ...
    public function checkSectionExist(){
        $ObjDB = new class_db();
        $S = "SELECT * FROM master_section_tbl where master_section_tbl.section_id = '".$this->section_id."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    public function checkMobileExist(){
        $ObjDB = new class_db();
        $S = "SELECT * FROM user_info_tbl where user_info_tbl.user_mobile = '".$this->user_mobile."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    public function updatePIN(){
        $ObjDB = new class_db();
        $S = "UPDATE user_info_tbl SET user_pin = '".$this->user_pin."' WHERE user_mobile = '".$this->user_mobile."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->ExecuteQuery();
        return $R;
    }
    ...
    public function getHardwareDetail(){
        ...
        $S = "SELECT hardware_schedule_config_tbl.next_schedule_date,master_status_tbl.status ,master_section_tbl.section_name,master_shop_tbl.shop_name,hardware_mapping_section_tbl.* ,master_hardware_category_tbl.category_name as hardware_category, master_hardware_type_tbl.hardware_type_name as hardware_type ,hardware_basic_tbl.*
                ...
                LEFT JOIN hardware_schedule_config_tbl ON hardware_schedule_config_tbl.map_id = hardware_mapping_section_tbl.map_id
                WHERE hardware_basic_tbl.hardware_id = '".$this->hardware_id."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    public function checkUserExist(){
        $ObjDB = new class_db();
        $S = "SELECT * FROM user_info_tbl where user_info_tbl.user_info_id = '".$this->user_info_id."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
