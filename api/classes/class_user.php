...
//    Preeti -14-11-2019----------
    ...
    //--15-11-2019--------------
    public function updateHardwareMappingTbl(){
        $ObjDB = new class_db();
        $date = date('Y-m-d');
        //$S = "UPDATE hardware_basic_tbl SET hardware_status = '60' WHERE hardware_id = '".$this->hardware_id."'";
        //$S = "UPDATE hardware_mapping_section_tbl SET map_status = '60', map_updated_date = '".$date."' WHERE map_id = '".$this->hardware_map_section_id."'";
        $S = "UPDATE hardware_mapping_section_tbl SET map_status = '".$this->case_type."', map_updated_date = '".$date."' WHERE map_id = '".$this->hardware_map_section_id."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->ExecuteQuery();
        return $R;
    }
...
//                . "AND hardware_basic_tbl.hardware_status = 10";
    ...
    public function getHardwareDetail(){
        $ObjDB = new class_db();
        $S = "SELECT hardware_schedule_config_tbl.next_schedule_date,master_status_tbl.status ,master_section_tbl.section_name,
                master_shop_tbl.shop_name,hardware_mapping_section_tbl.* ,master_hardware_category_tbl.category_name as hardware_category, 
                master_hardware_type_tbl.hardware_type_name as hardware_type ,hardware_basic_tbl.*
    ...
    public function ticketListShop() {
        ...
        $S = "SELECT master_status_tbl.status,master_maintenance_section_tbl.maintenance_section_name,"
                ...
                . "LEFT JOIN master_status_tbl ON master_status_tbl.status_code = tickets_tbl.ticket_status "
                . "LEFT JOIN master_section_tbl ON master_section_tbl.section_id = tickets_tbl.section_id "
                . "WHERE tickets_tbl.shop_id IN (SELECT user_mapping_tbl.shop_id "
                . "FROM user_mapping_tbl "
                . "WHERE user_mapping_tbl.user_info_id = '".$this->user_info_id."') "
                . "ORDER BY tickets_tbl.tickets_created_date DESC";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    ...
    public function ticketListSection(){
        ...
        $S = "SELECT master_status_tbl.status,master_maintenance_section_tbl.maintenance_section_name,"
                ...
                . "LEFT JOIN master_status_tbl ON master_status_tbl.status_code = tickets_tbl.ticket_status "
                . "LEFT JOIN master_section_tbl ON master_section_tbl.section_id = tickets_tbl.section_id "
                . "WHERE tickets_tbl.section_id IN (SELECT user_mapping_tbl.section_id "
                . "FROM user_mapping_tbl "
                . "WHERE user_mapping_tbl.user_info_id = '".$this->user_info_id."') "
                . "ORDER BY tickets_tbl.tickets_created_date DESC";
        $ObjDB->sproc_name = $S;
    ...
    public function ticketListMaintenanceSection(){
        ...
        $S = "SELECT master_status_tbl.status,master_maintenance_section_tbl.maintenance_section_name,"
                ...
                . "LEFT JOIN master_status_tbl ON master_status_tbl.status_code = tickets_tbl.ticket_status "
                . "LEFT JOIN master_section_tbl ON master_section_tbl.section_id = tickets_tbl.section_id "
                . "WHERE tickets_tbl.maintenance_section_id  IN (SELECT user_mapping_tbl.maintenance_section_id  "
                . "FROM user_mapping_tbl WHERE user_mapping_tbl.user_info_id = '".$this->user_info_id."') "
                . "ORDER BY tickets_tbl.tickets_created_date DESC";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    ...
    public function ticketListDivAdmin(){
        ...
        $S = "SELECT master_status_tbl.status,master_maintenance_section_tbl.maintenance_section_name,"
                ...
                . "LEFT JOIN master_status_tbl ON master_status_tbl.status_code = tickets_tbl.ticket_status "
                . "LEFT JOIN master_section_tbl ON master_section_tbl.section_id = tickets_tbl.section_id "
                . "WHERE tickets_tbl.maintenance_section_id  IN "
                . "(SELECT user_mapping_tbl.maintenance_section_id  "
                . "FROM user_mapping_tbl WHERE user_mapping_tbl.user_info_id = '".$this->user_info_id."') "
                . "ORDER BY tickets_tbl.tickets_created_date DESC";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
    ...
    public function ticketStatusCountMainShop(){
        $ObjDB = new class_db();
        $S = "SELECT IFNULL(SUM(CASE WHEN tickets_tbl.ticket_status = '20' THEN 1 ELSE 0 END),0) AS 'Open' , "
                ...
                . "IFNULL(SUM(CASE WHEN tickets_tbl.tickets_created_date = '".$this->date."' AND tickets_tbl.ticket_status != '50' THEN 1 ELSE 0 END),0) AS 'fresh', "
                . "IFNULL(SUM(CASE WHEN tickets_tbl.tickets_created_date < '".$this->date."' AND tickets_tbl.ticket_status != '50' THEN 1 ELSE 0 END),0) AS 'pending', "
                . "IFNULL(SUM(CASE WHEN tickets_tbl.ticket_status = '50' THEN 1 ELSE 0 END),0) AS 'Close', "
                . "COUNT(*) as Total "
                . "FROM tickets_tbl "
...
//                . "AND hardware_basic_tbl.hardware_status = 10 "
    ...
    $ObjDB = new class_db();
    $S = "SELECT um.user_info_id, user_device_id FROM user_mapping_tbl um LEFT JOIN 
            user_device_tbl ud ON um.user_info_id = ud.user_info_id
            WHERE um.maintenance_shop_id = '".$this->maintenance_shop_id."' LIMIT 0,1";
    $ObjDB->sproc_name = $S;
    $R = $ObjDB->SelectQuery();
    return $R;
}
    ...
    public function checkLogin(){
        $ObjDB = new class_db();
        $S = "SELECT * FROM user_info_tbl WHERE user_mobile = '".$this->user_mobile."' "
                . "AND user_status = '80' OR user_status = '0'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    ...
    //preeti - 27-11-2019 
    public function GetDueHardwareList(){
        $ObjDB = new class_db();
        $S = "SELECT master_shop_tbl.shop_name as shop_name, "
                . "master_section_tbl.section_name as section_name, "
                . "hardware_schedule_config_tbl.*, "
    ...
    public function getSectionCount(){
        $ObjDB = new class_db();
        $S = "SELECT COUNT(*)as sectionCount FROM `master_section_tbl` WHERE shop_id = '".$this->shop_id."' AND `section_status` = '10'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    ...
    //Srishti 11-Dec-2019
    public function getHardwareCountActive(){
        $RES = 0;
        $ObjDB = new class_db();
        //$S = "SELECT COUNT(*) as totalHardware FROM hardware_mapping_section_tbl WHERE hardware_mapping_section_tbl.shop_id = '".$this->shop_id."' AND hardware_mapping_section_tbl.map_status = '10' ";
        $S = "SELECT COUNT(*) as activeHardware FROM hardware_mapping_section_tbl WHERE "
                . " hardware_mapping_section_tbl.shop_id = '".$this->shop_id."' AND hardware_mapping_section_tbl.map_status = '10' ";
        /*$S = "SELECT "
                . "IFNULL(SUM(CASE WHEN hardware_mapping_section_tbl.map_status = '10' THEN 1 ELSE 0 END),'0') as 'activeHardware',"
                . "COUNT(*) as totalHardware "
                . "FROM hardware_mapping_section_tbl "
                . "WHERE hardware_mapping_section_tbl.shop_id = '3' "
                . "AND hardware_mapping_section_tbl.map_status != '80' ";*/
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        if(!empty($R) && mysqli_num_rows($R)){
            list($RES) = mysqli_fetch_row($R);
        }
        return $RES;
    }
    ...
    public function getHardwareCount(){
        $ObjDB = new class_db();
        //$S = "SELECT COUNT(*) as totalHardware FROM hardware_mapping_section_tbl WHERE hardware_mapping_section_tbl.shop_id = '".$this->shop_id."' AND hardware_mapping_section_tbl.map_status = '10' ";
        $S = "SELECT COUNT(*) as totalHardware FROM hardware_mapping_section_tbl WHERE "
                . " hardware_mapping_section_tbl.shop_id = '".$this->shop_id."' AND hardware_mapping_section_tbl.map_status != '80' ";
        /*$S = "SELECT "
                . "IFNULL(SUM(CASE WHEN hardware_mapping_section_tbl.map_status = '10' THEN 1 ELSE 0 END),'0') as 'activeHardware',"
                . "COUNT(*) as totalHardware "
                . "FROM hardware_mapping_section_tbl "
    ...
    public function getDeviceId(){
        $ObjDB = new class_db();
        $S = "SELECT * FROM user_device_tbl WHERE user_device_tbl.user_info_id = '".$this->user_info_id."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    //10-12-2019
    public function getShopInchDetail(){
        $ObjDB = new class_db();
        $S = "SELECT user_mapping_tbl.*,user_details_tbl.*,user_info_tbl.* "
                . "FROM user_mapping_tbl "
                . "LEFT JOIN user_info_tbl ON user_mapping_tbl.user_info_id = user_info_tbl.user_info_id "
                . "LEFT JOIN user_details_tbl ON user_mapping_tbl.user_info_id = user_details_tbl.user_info_id "
                . "WHERE user_mapping_tbl.shop_id = '".$this->shop_id."' "
                . "AND user_mapping_tbl.section_id IS NULL";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    public function getMainShopInchDetail(){
        $ObjDB = new class_db();
        $S = "SELECT user_mapping_tbl.*,user_details_tbl.*,user_info_tbl.* "
                . "FROM user_mapping_tbl "
                . "LEFT JOIN user_info_tbl ON user_mapping_tbl.user_info_id = user_info_tbl.user_info_id "
                . "LEFT JOIN user_details_tbl ON user_mapping_tbl.user_info_id = user_details_tbl.user_info_id "
                . "WHERE user_mapping_tbl.`maintenance_shop_id` = '".$this->maintenance_shop_id."' "
                . "AND user_mapping_tbl.`maintenance_section_id` IS NULL";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    public function getTicketCountForMain(){
        $ObjDB = new class_db();
        $S = "SELECT IFNULL(SUM(CASE WHEN tickets_tbl.ticket_status = '20' THEN 1 ELSE 0 END),0) AS 'Open' , "
                . "IFNULL (SUM(CASE WHEN tickets_tbl.case_type = '60' THEN 1 ELSE 0 END),0) AS 'Under Maintenance', "
                . "IFNULL(SUM(CASE WHEN tickets_tbl.case_type = '90' THEN 1 ELSE 0 END),0) AS 'Breakdown', "
                . "IFNULL(SUM(CASE WHEN tickets_tbl.ticket_status = '50' THEN 1 ELSE 0 END),0) AS 'Close' , "
                . "COUNT(*) as Total "
                . "FROM tickets_tbl "
                . "WHERE tickets_tbl.maintenance_shop_id = '".$this->maintenance_shop_id."'";
    ...
    // 11-12-2019
    public function getCategoryMainShop(){
        $ObjDB = new class_db();
        $today = date('Y-m-d');
        $S = "SELECT IFNULL(SUM(CASE WHEN tickets_tbl.ticket_status = '20' THEN 1 ELSE 0 END),0) AS 'Open' , "
                . "IFNULL (SUM(CASE WHEN tickets_tbl.case_type = '60' AND tickets_tbl.ticket_status != '50' THEN 1 ELSE 0 END),0) AS 'Under Maintenance', "
                ...
                . "AND tickets_tbl.maintenance_shop_id = '".$this->maintenance_shop_id."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    public function getMaintenanceSectionCount(){
        $ObjDB = new class_db();
        $S = "SELECT COUNT(*) as sectionCount "
