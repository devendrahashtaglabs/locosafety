...
class class_maintenance {
    ...
    public $schedule_updated_date;
    public $schedule_updated_by;
    public $map_id;
    public $service_date, $hardware_cycle, $monthly_type, $monthly_date, $monthly_week, $monthly_name,
            $monthly_week_day, $weekly_day, $weekly_recur_every, $daily_every_day;
...
//        echo $S;exit;
    ...
    public function updateTicketLogFixed(){
        $ObjDB = new class_db();
        $today = date('Y-m-d');
        $S = "INSERT INTO tickets_log_tbl (`ticket_status`, `replacement_remarks`,`log_update_date`,`log_updated_by`,"
                . "`ticket_no`) VALUES ('50','".$this->case_remarks."','".$today."','".$this->user_info_id."',"
                . "'".$this->ticket_no."')";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->ExecuteQuery();
        return $R;
...
//        echo $S;exit;
    ...
    public function getScheduleDetail(){
        $ObjDB = new class_db();
        $S = "SELECT hardware_schedule_mapping_tbl.* "
                . "FROM hardware_schedule_mapping_tbl "
                . "WHERE `hardware_section_map_id` "
                . "IN (SELECT `hardware_map_section_id` "
                . "FROM tickets_tbl WHERE `ticket_no` = '".$this->ticket_no."')";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->SelectQuery();
        return $R;
    }
    public function updateNextScheduleDate(){
        $ObjDB = new class_db();
        $today = date('Y-m-d');
        $S = "UPDATE hardware_schedule_config_tbl SET schedule_start_date = '".$today."', next_schedule_date = '".$this->next_schedule_date."', schedule_updated_date = '".$today."', schedule_updated_by = '".$this->user_info_id."' WHERE map_id = '".$this->map_id."'";
        $ObjDB->sproc_name = $S;
        $R = $ObjDB->ExecuteQuery();
        return $R;
    }
    ...
    public function GetNextScheduleDate() {
        $nextScheduleDate = '';
        $seviceDate = date('Y-m-d');
        if ($this->hardware_cycle == '1') {
            $Days = $this->daily_every_day;
            $nextScheduleDate = date('Y-m-d', strtotime($seviceDate . ' + ' . $Days . ' days'));
        } elseif ($this->hardware_cycle == '2') {
            $nextScheduleDate = date('Y-m-d', strtotime($this->weekly_day, strtotime('this week +' . $this->weekly_recur_every . ' week')));
        } elseif ($this->hardware_cycle == '3') {
            if ($this->monthly_type == 'day') {
                $NextMonthYear = date('M Y', strtotime($seviceDate . ' + 1 month'));
                $nextScheduleDate = date('Y-m-d', strtotime($this->monthly_date . ' ' . $NextMonthYear));
            } elseif ($this->monthly_type == 'monthly') {

                $NextMonthYear = date('M Y', strtotime($seviceDate . ' + ' . $this->monthly_name . ' month'));
                if ($this->monthly_week == '1') {
                    $nextScheduleDate = date('Y-m-d', strtotime("1 " . $NextMonthYear . " " . "this weeks " . $this->monthly_week_day . ""));
                } else if ($this->monthly_week == '5') {
                    $yearNew = date('Y', strtotime($NextMonthYear));
                    $monthNew = date('m', strtotime($NextMonthYear));
                    $nextScheduleDate = $this->getLastMonday($monthNew, $yearNew, $this->monthly_week_day);
                } else {
                    $weekDays = $this->monthly_week - 1;
                    $nextScheduleDate = date('Y-m-d', strtotime("1 " . $NextMonthYear . " " . "+ " . $weekDays . " weeks " . $this->monthly_week_day . ""));
                }
            }
        }
//        echo $nextScheduleDate;exit;
        return $nextScheduleDate;
    }

    public function getLastMonday($month, $year, $day) {
