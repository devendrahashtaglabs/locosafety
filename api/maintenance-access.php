...
if ($api_key == "501") {
    ...
    switch ($action) {
        ...
        case "closeTicketFixedHardware":
            ...
            $T = $OTh->closeTicket();
            $OTh->updateTicketLogFixed();//preeti- 29-11-2019
            ...
            //select hardware_schedule_mapping_tbl
            $sch = $OTh->getScheduleDetail();
            if($sch){
                while ($row1 = mysqli_fetch_array($sch)) {
                    $OTh->hardware_section_map_id = $row1['hardware_section_map_id'];
                    $OTh->hardware_id = $row1['hardware_id'];
                    $OTh->hardware_cycle = $row1['hardware_cycle'];
                    $OTh->daily_every_day = $row1['daily_every_day'];
                    $OTh->weekly_recur_every = $row1['weekly_recur_every'];
                    $OTh->weekly_day = $row1['weekly_day'];
                    $OTh->monthly_type = $row1['monthly_type'];
                    $OTh->monthly_date = $row1['monthly_date'];
                    $OTh->monthly_name = $row1['monthly_name'];
                    $OTh->monthly_week = $row1['monthly_week'];
                    $OTh->monthly_week_day = $row1['monthly_week_day'];
                    $OTh->service_date = date('Y-m-d');
                $NxtDate = $OTh->GetNextScheduleDate();
                if($NxtDate){
                    $OTh->map_id = $row1['hardware_section_map_id'];
                    $OTh->next_schedule_date = $NxtDate;
                    $OTh->updateNextScheduleDate();
                    }
                }
            } 
            $receiver_id = '';
        ...
        case "closeTicketNotFixedHardware":
            ...
            $T = $OTh->closeTicket();
            $OTh->updateTicketLogNotFixed();//Preeti- 29-11-2019
            ...
            //05-12-2019 hardware_schedule_mapping_tbl
            $sch = $OTh->getScheduleDetail();
            if($sch){
                while ($row1 = mysqli_fetch_array($sch)) {
                    $OTh->hardware_section_map_id = $row1['hardware_section_map_id'];
